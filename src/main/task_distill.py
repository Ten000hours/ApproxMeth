...
#from transformer.roberta_modeling import TinyRobertaForSequenceClassification
...
class ImdbProcessor(DataProcessor):
    def __init__(self,):
        ...
        self.raw_datasets = load_dataset("imdb")
    ...
    def get_dev_examples(self, data_dir):
        ...
        return self._create_examples(test_dataset, "eval")
    
    def get_labels(self, ):
        return [0, 1]
        # return self.raw_datasets["train"].features["label"].names
...
def main():
    ...
    if "roberta" in args.teacher_model:
        tokenizer = AutoTokenizer.from_pretrained("roberta-base", do_lower_case=args.do_lower_case)
    else:
    ...
    if not args.do_eval:
        if "roberta" in args.teacher_model:
            teacher_model = TinyRobertaForSequenceClassification.from_pretrained(args.teacher_model, num_labels=num_labels)
        else:
            teacher_model = TinyBertForSequenceClassification.from_pretrained(args.teacher_model, hidden_act="gelu", softmax_act="softmax", num_labels=num_labels)
